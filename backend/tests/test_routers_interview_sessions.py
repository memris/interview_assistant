...
def test_create_interview_session():
    user_resp = client.post("/api/auth/", json={"username": "testuser", "email": "test@example.com", "password": "pass", "role": "candidate"})
    topic_resp = client.post("/api/topics/", json={"topic_name": "Test Topic"})
    user_id = user_resp.json()["id"]
    topic_id = topic_resp.json()["id"]
    
    response = client.post("/api/interview-sessions/", json={"user_id": user_id, "topic_id": topic_id})
    assert response.status_code == 200
    assert response.json()["user_id"] == user_id


def test_generate_interview_question():
    user_resp = client.post("/api/auth/", json={"username": "questionuser", "email": "question@example.com", "password": "pass", "role": "candidate"})
    topic_resp = client.post("/api/topics/", json={"topic_name": "Question Topic"})
...
def test_create_interview_session_bad_topic():
    user_resp = client.post("/api/auth/", json={"username": "invalidtopicuser", "email": "invalidtopic@example.com", "password": "pass", "role": "candidate"})
    user_id = user_resp.json()["id"]
